...
export default function CheckEmailScreen({ navigation }: any) {
  ...
  const handleContinue = async () => {
    try {
      ...
      if (userError || !user) {
        Alert.alert('Still waiting…', 'Please confirm your email first.')
        return
      }
  ...
  return (
    <View style={styles.container}>
      ...
      <Button title={continuing ? 'Continuing…' : 'Continue to App'} onPress={handleContinue} disabled={continuing} />
      <Button title={resending ? 'Resending…' : 'Resend Email'} onPress={handleResend} disabled={resending} />
