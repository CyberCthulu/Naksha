...
| Finding | Correction |
| --- | --- |
| Today's Energy used the raised card color while Weekly Forecast used the standard card color. | Both use the same `GuidanceCard` wrapper and opaque standard surface, with shared title, metadata, and expand/collapse styles. |
...
The background uses the existing SVG dependency and has no animation loop,
timers, or new runtime assets. Reading cards stay opaque to keep their colors
and text contrast consistent. Quiet form/reading backgrounds retain their
subdued gradient.
